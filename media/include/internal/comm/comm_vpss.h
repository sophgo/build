...
/*
 * mode: decide vpss work as 1/2 device.
 * input: decide the input of each vpss device.
 */
typedef struct _vpss_mode_s {
	vpss_mode_e mode;
	vpss_input_e input[VPSS_DEVICE_NUM];
} vpss_mode_s;
...
typedef struct _vpss_all_proc_amp_s {
	int proc_amp[VPSS_MAX_GRP_NUM][PROC_AMP_MAX];
} vpss_all_proc_amp_s;
