 ...
 *****************************************************************************/
static inline void Rgb2Yc(unsigned short r, unsigned short g, unsigned short b, unsigned short *py, unsigned short *pcb, unsigned short *pcr)
{
	/* Y */
	*py = (unsigned short)((((r * 66 + g * 129 + b * 25) >> 8) + 16) << 2);

	/* Cb */
	*pcb = (unsigned short)(((((b * 112 - r * 38) - g * 74) >> 8) + 128) << 2);

	/* Cr */
	*pcr = (unsigned short)(((((r * 112 - g * 94) - b * 18) >> 8) + 128) << 2);
}

static inline unsigned int Rgb2Yuv(unsigned int u32Rgb)
{
...
static inline void Rgb2Yc_full(unsigned short r, unsigned short g, unsigned short b, unsigned short *py, unsigned short *pcb, unsigned short *pcr)
{
	unsigned short py_temp, pcb_temp, pcr_temp;

	py_temp = (unsigned short)(((r * 76 + g * 150 + b * 29) >> 8) * 4);
	pcb_temp = (unsigned short)(CLIP_MIN(((((b * 130 - r * 44) - g * 86) >> 8) + 128), 0) * 4);
	pcr_temp = (unsigned short)(CLIP_MIN(((((r * 130 - g * 109) - b * 21) >> 8) + 128), 0) * 4);
...
static inline unsigned int Rgb2Yuv_full(unsigned int u32Rgb)
{
	unsigned short y, u, v;
 ...
 *****************************************************************************/
static inline void Rgb2Yc_8BIT(unsigned char r, unsigned char g, unsigned char b, unsigned char *py, unsigned char *pcb, unsigned char *pcr)
{
	/* Y */
...
static inline unsigned int Rgb2Yuv_8BIT(unsigned int u32Rgb)
{
	unsigned char y, u, v;
...
static inline void Rgb2Yc_full_8BIT(unsigned char r, unsigned char g, unsigned char b, unsigned char *py, unsigned char *pcb, unsigned char *pcr)
{
	short py_temp, pcb_temp, pcr_temp;
...
static inline unsigned int Rgb2Yuv_full_8BIT(unsigned int u32Rgb)
{
	unsigned char y, u, v;
...
static inline void InitFps(FPS_CTRL_S *pFrmCtrl, unsigned int u32FullFps, unsigned int u32TagFps)
{
...
static inline unsigned char FpsControl(FPS_CTRL_S *pFrmCtrl)
{
...
static inline unsigned int GetLowAddr(uint64_t u64Phyaddr)
{
	return (unsigned int)u64Phyaddr;
}

static inline unsigned int GetHighAddr(uint64_t u64Phyaddr)
{
