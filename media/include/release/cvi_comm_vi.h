...
typedef struct _VI_GPIO_CFG_S {
	CVI_BOOL bEnable;/*enable mipi switch*/
	CVI_S32 s32GpioPort;/*gpio port*/
	CVI_S32 s32GpioPin;/*gpio pin*/
	CVI_S32 s32GpioPol;/*gpio initial state, 0 pull down, 1 pull up*/
} VI_GPIO_CFG_S;
...
typedef struct _VI_DEV_ATTR_EX_S {
	CVI_BOOL bMuxDev; /* multi sensor use same dev*/
	CVI_U8 u8SnsrNum; /* the num of multi sensor */
	VI_DEV phyDev; /* bind dev, must phy dev [0 ~ VI_MAX_PHY_DEV_NUM)*/
	VI_GPIO_CFG_S stGpioCfg[VI_MAX_DEV_SWITCH_DEPTH];/*default gpio info*/
	CVI_BOOL bFrmCtrl; /*Frame ctrl for mipi switch */
	CVI_U8 u8DstFrm; /*revice frm_num after switch */
} VI_DEV_ATTR_EX_S;
...
typedef struct _VI_CHN_ATTR_S {
	...
	CVI_BOOL bSingleVb; /*RW; single vb or ping-pong vb*/
} VI_CHN_ATTR_S;
...
#ifdef __arm__
	CVI_U32 padding; /* padding for keeping same size of this structure */
#endif
	RECT_S  stCropRect;
