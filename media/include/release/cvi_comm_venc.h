...
typedef struct _VENC_PACK_S {
	...
	CVI_U64 u64RingBufBasePhyAddr; /* R; the physics address of Ring Buf base*/
	CVI_U32 u32RingBufLen; /* R; the total len of ring buf size*/
} VENC_PACK_S;
...
typedef struct _VENC_ATTR_S {
	...
	CVI_U32 u32Profile;			///< profie setting for video encoder(h264/h265)
...
typedef struct _USER_RC_INFO_S {
	...
	CVI_U64 u64AiMapPhyAddr; /* RW; Physical address of the QP table in ai QpMap mode*/
