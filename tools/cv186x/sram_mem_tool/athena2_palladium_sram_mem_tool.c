...
int main(int argc, char *argv[])
{
    ...
    case 0: // load
        ...
        for (i = 0; i < tb_num; i++)
        {
            ...
            strncpy(fn, argv[4], n);
            if (interleave)
                sprintf(fn+n, "_sram%d", i);
            else
                sprintf(fn+n, "_sram%d", (int)(address_backup >> 32));
    ...
    if (opt == 0 || opt == 2) // load.tcl
    {
        ...
        for (j = 0; j < tb_num; j++)
        {
            ...
            //{
            //    bank_size[i] = (bank_size[i] + MAX_OUTFILE_NUM*2 -1) ;
            //    printf("1:bank_size[%d]=%d.\n", i, bank_size[i]);
            //}
            ...
            for (i = start_bank; i < start_bank + max_bank; i++)
            {
                ...
                switch(j){
                    case 0:
                        //printf("bank size = %d\n", bank_size[i]);
                        for (k = 0; k <= bank_size[i] / 4096; k++)
                            fprintf(fin, "memory -load %%readmemh {u_chip_top.chip_core.A_tpu_sys_pr_wrap.u_tpu_sys.u_tpu_sys.u_tpu0.u_tpu_pwr_wrap.u_tpu.arrays_grp.\\gen_arrays[0].genblk1.u_array .u_array_pr_wrap.u_array_pwr_wrap.u_array_wrap.u_array.u_qlane.\\gen_multi_lane[0].u_lane .lmem.u_lmem_slice.\\gen_sram_bank[%d].u_lmem_bank .\\gen_sram_slice[%d].u_ram0 .u_mem_u_tpu_lmem_0.ram_core} -file %s%s_sram%d_bank%d_slice%d_mem0.h  -start 0x%lx -end 0x%lx\n",\
                                    k, i, prefix, fn, j, k, id, (unsigned long)(address + offset), \
                                    (unsigned long)(bank_size[i] >= 4096*(k+1) ? 4096/16 - 1 : bank_size[i]%4096/16));
                        #if 0
                        switch(i & 0x1){
                        case 0:
                            ...
                            fprintf(fin, "memory -load %%readmemh {u_chip_top.chip_core.A_tpu_sys_pr_wrap.u_tpu_sys.u_tpu_sys.u_tpu0.u_tpu_pwr_wrap.u_tpu.arrays_grp.\\gen_arrays[0].genblk1.u_array .u_array_pr_wrap.u_array_pwr_wrap.u_array_wrap.u_array.u_qlane.\\gen_multi_lane[0].u_lane .lmem.u_lmem_slice.\\gen_sram_bank[0].u_lmem_bank .\\gen_sram_slice[0].u_ram0 .u_mem_u_tpu_lmem_0.ram_core} -file %s%s_sram%d_slice%d_mem0.h  -start 0x%lx -end 0x%lx\n",\
                        ...
                        case 1:
                            ...
                            fprintf(fin, "memory -load %%readmemh {u_chip_top.chip_core.A_tpu_sys_pr_wrap.u_tpu_sys.u_tpu_sys.u_tpu0.u_tpu_pwr_wrap.u_tpu.arrays_grp.\\gen_arrays[0].genblk1.u_array .u_array_pr_wrap.u_array_pwr_wrap.u_array_wrap.u_array.u_qlane.\\gen_multi_lane[0].u_lane .lmem.u_lmem_slice.\\gen_sram_bank[0].u_lmem_bank .\\gen_sram_slice[1].u_ram0 .u_mem_u_tpu_lmem_0.ram_core} -file %s%s_sram%d_slice%d_mem0.h  -start 0x%lx -end 0x%lx\n",\
                                    prefix, fn, j, id, (unsigned long)(address + offset), \
                                    (unsigned long)address + offset + bank_size[id] - 1);
                            break;
                        }
...
long create_mem_image(char *filename, unsigned char *out, unsigned long long address, unsigned long max_size)
{
    ...
    for (i = 0; i < MAX_OUTFILE_NUM; i++)
    {
        for (j = start_bank; j < start_bank + max_bank; j++)
        {
            ...
            sprintf(fn, "%s_slice%d_mem%d.h", filename, id, i);
            fp[id][i] = fopen(fn, "rt");
            if (fp[id][i] == NULL)
            {
                printf("open file %s failed\n", fn);
...
long create_palladium_file(unsigned char *src, char *filename, unsigned long long address, unsigned long max_size)
{
    ...
    for (i = 0; i <= max_size/8192; i++)
    {
        for (j = start_bank; j < max_bank + start_bank; j++)
        {
            int id = j & MAX_BANK_MASK;
            sprintf(fn, "%s_bank%d_slice%d_mem0.h", filename, i, id);
            fp[id][i] = fopen(fn, "wt");
            if (fp[id][i] == NULL)
            {
    ...
    for (i = start_bank; i < start_bank + max_bank; i++)
    {
        ...
        for (j = 0; j < dstSize[id];)
        {
            fid = j / 4096;
            //printf("j=%d, fid = %d.\n", j, fid);
            fprintf(fp[id][fid], "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n", pDst[id][15], pDst[id][14], pDst[id][13], pDst[id][12], pDst[id][11], pDst[id][10], pDst[id][9], pDst[id][8], pDst[id][7], pDst[id][6], pDst[id][5], pDst[id][4], pDst[id][3], pDst[id][2], pDst[id][1], pDst[id][0]);
            pDst[id] += 16;
            j += 16;
