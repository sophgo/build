...
class ImagerBuilder(object):
    ...
    def packHeader(self, part):
        ...
        with open(part["file_path"], "rb") as fd:
            ...
            file_size = os.stat(part["file_path"]).st_size
            if file_size > part["file_size"]:
                logging.error(
                    "Image: %s(%d) is larger than partition size(%d)"
                    % (part["file_name"], file_size, part["file_size"])
                )
                raise OverflowError
    ...
    def _getChunkHeader(self, size: int, offset: int, part_sz: int, crc32: int):
        ...
        """
        logging.info(
            "size:%x, offset:%x, part_sz:%x, crc:%x" % (size, offset, part_sz, crc32)
        )
        Chunk = array(
            "I",
            [
