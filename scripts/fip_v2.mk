...
opensbi: export SUSPEND=${CONFIG_SUSPEND}
...
fsbl-build: u-boot-build memory-map ${BL2_CVIPART_DEP}
	...
	${Q}$(MAKE) -j${NPROC} -C ${FSBL_PATH} O=${FSBL_OUTPUT_PATH} LOG_LEVEL=${LOG_LEVEL} BLCP_2ND_PATH=${BLCP_2ND_PATH} \
		...
		SWITCH_32K_XTAL=$(CONFIG_SWITCH_32K_XTAL) \
		FORCE_BOOT_FROM_FLASH=y ENABLE_UART_DL=${CONFIG_ENABLE_UART_DL}
...
ifeq (${CONFIG_UBOOT_SPL_CUSTOM},y)
	...
	${Q}$(MAKE) -j${NPROC} -C ${FSBL_PATH} O=${FSBL_OUTPUT_PATH} LOG_LEVEL=${LOG_LEVEL} BLCP_2ND_PATH=${BLCP_2ND_PATH} \
		...
		CONFIG_MMC_SKIP_TUNING=$(CONFIG_MMC_SKIP_TUNING) ENABLE_UART_DL=n
