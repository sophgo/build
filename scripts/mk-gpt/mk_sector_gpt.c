...
static void init_gpt(uint64_t total_sectors)
{
	uint64_t sector = total_sectors;
	uuid_t uuid;
...
static void write_partition(int fd, xmlNodePtr xml_node)
{
	...
	prop = xmlGetProp(xml_node, BAD_CAST "size_in_sectors");
	if (prop != NULL) {
		size = (uint64_t)atoll((char *)prop);
		xmlFree(prop);
	}
	...
	while (xml_node != NULL) {
		if (xml_node->type == XML_ELEMENT_NODE)
			if (strcmp("physical_partition",
				...
				if (prop != NULL) {
					size = (uint64_t)atoll((char *)prop);
					printf("[hq] total size:%lu sectors\n", size);
					xmlFree(prop);
				}
				break;
			}
