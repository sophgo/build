...
def parse_mmap(mmap_module):
    ...
    for attr in dir(mmap):
        if attr.startswith("_"):
            continue

        value = getattr(mmap, attr)
        # skip methods and non-numeric types
        if callable(value) or not isinstance(value, (int, float)):
            continue
...
def main():
    ...
    # parser.add_argument("--os", choices=["dual-os", "linux"], required=True)
    ...
    # create instance to execute __init__
    _ = mmap_module.MemoryMap()
