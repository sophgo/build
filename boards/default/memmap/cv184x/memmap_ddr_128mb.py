...
class MemoryMap:
    ...
    FSBL_C906L_START_ADDR = DRAM_BASE + 1 * SIZE_1M
    ...
    RTOS_LOGO_ADDR = RTOS_COMPRESS_BIN_ADDR
    # RTOS ION memory
    ...
    def __init__(self):
        ...
        if rtos_sys_size == '0x00000000':
            setattr(MemoryMap, 'RTOS_SYS_TOTAL_SIZE', 0)
        else:
            setattr(MemoryMap, 'RTOS_SYS_TOTAL_SIZE',
                    int(rtos_sys_size, 16)
                    (+ getattr(MemoryMap, 'RTOS_LOG_SIZE') + getattr(MemoryMap, 'SHARE_MEM_SIZE'))
                    (+ (getattr(MemoryMap, 'SHARE_PARAM_SIZE') * 2) + getattr(MemoryMap, 'PQBIN_SIZE')))

        if getattr(MemoryMap, 'RTOS_SYS_TOTAL_SIZE') > 0:
        ...
        setattr(MemoryMap, 'BOOTLOGO_ADDR',
                getattr(MemoryMap, 'ISP_MEM_BASE_ADDR') + getattr(MemoryMap, 'ISP_MEM_BASE_SIZE'))
        if getattr(MemoryMap, 'RTOS_SYS_TOTAL_SIZE') > 0:
